    ...
    public partial class MissileManager : Node3D
    {
        ...
        private void SetupMultiMesh()
        {
            ...
            try 
            {
                ...
                var scene = GD.Load<PackedScene>("res://missile.glb");
                if (scene != null)
                {
                    ...
                    {
                        GD.PrintErr("MissileManager: Could not find MeshInstance3D in missile.glb. Using Cylinder.");
                    ...
                    else
                    {
                         ...
                         GD.Print("MissileManager: Successfully loaded mesh from missile.glb");
                    }
                    instance.QueueFree();
                }
                else
                {
                     throw new Exception("Failed to load res://missile.glb");
                }
            }
            catch (Exception e)
            {
        ...
        public void SpawnMissile(Node launcher, Node3D target, Vector3 startPos, Quaternion startRot, Vector3 initialVelocity, int volleyIndex)
        {
            // Find free slot
            int index = -1;
        ...
        public override void _PhysicsProcess(double delta)
        {
            ...
            for (int i = 0; i < MAX_MISSILES; i++)
            {
                ...
                if (_missiles[i].Active)
                {
                    ...
                    Quaternion visualRot = _missiles[i].Rotation * Quaternion.FromEuler(new Vector3(0, -Mathf.Pi / 2, 0));
        ...
        private void UpdateMissile(int i, float dt, PhysicsDirectSpaceState3D spaceState)
        {
            ...
            if (_missiles[i].EngineIgnited && _missiles[i].Target != null && GodotObject.IsInstanceValid(_missiles[i].Target))
            {
