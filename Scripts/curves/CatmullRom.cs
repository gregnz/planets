...
public class CatmullRomSpline
{
	...
	{
		DebugDraw3D.DrawPoints(controlPointsList, 0.4f, Colors.Blue);
		...
		for (int i = 0; i < controlPointsList.Length; i++)
		{
			...
			if (( i == controlPointsList.Length - 2 || i == controlPointsList.Length - 1) && !isLooping)
			{
				continue;
			}
	...
	void DisplayCatmullRomSpline(int pos)
	{
		...
		for (int i = 1; i <= loops; i++)
		{
			...
			DebugDraw3D.DrawLine(lastPos, newPos, Colors.Lavender);
